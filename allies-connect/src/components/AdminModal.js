                ...
                <tbody>
                    {pendingOrgs.map((org) => (
                        <tr key={org.provider_id} className="text-center align-middle">
                            ...
                            <td>{org.ein}</td>
                            <td>
                                <button className="outline-warning me-2" onClick={() => setDetailModalType("pendingOrgs")}>
...
function AdminModal({ show, onHide, type}) {
    ...
    return (
        <Modal show={show} className="modal-wide" onHide={onHide}>
            ...
            <AdminDetailsModal show={!!detailModalType} type={detailModalType} onHide={() => setDetailModalType("")}/>
        </Modal>
    );
}
